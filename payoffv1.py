...
# round values
#df = df.round(2)
#df['period_additional_payment'] = np.random.randint(2100, 4500, size=len(df))
df
...
#df=df.drop(df.index[df['Ending Balance'] > 0], inplace=True)
df_mask=df['Ending Balance']>= 18000
filtered_df = df[df_mask].round(2)
filtered_df.to_csv('amort.csv')